...
describe("Registering a new user", () => {
  ...
  it("should register a new user", async () => {
    const res = await chai.request(server).put(route + openSessionID + '/user').query({ username: 'toto' })
  ...
  it("should not register a new user to running session", async () => {
    const res = await chai.request(server).put(route + runningSessionID + '/user').query({ username: 'toto' })
  ...
  it("should not register a new user to closed session", async () => {
    const res = await chai.request(server).put(route + closedSessionID + '/user').query({ username: 'toto' })
  ...
  it("should not register a new user to sessions that does not exist", async () => {
    const id = uuid()
    const res = await chai.request(server).put(route + id + '/user').query({ username: 'toto' })
  ...
  it("should not register a new user with missing username", async () => {
    const id = uuid()
    const res = await chai.request(server).put(route + id + '/user').query({ })

    expect(res.status).to.eql(400)
    expect(res.body.message).to.eql('request.query should have required property \'username\'');
  })

  it("should not register a new user already existing username", async () => {
    await chai.request(server).put(route + openSessionID + '/user').query({ username: 'tutu' })
    const res = await chai.request(server).put(route + openSessionID + '/user').query({ username: 'tutu' })
