...
describe("Check bids route input validation", () => {
  after(async () => {
    await server.close();
  });
  ...
  it("should failed with missing volume", async () => {
    ...
    expect(res.body.message).to.eql(
      "request.body.bid should have required property 'volume_mwh'"
    );
  });
  ...
  it("should failed with non number volume", async () => {
    ...
    expect(res.status).to.eql(400);
    expect(res.body.message).to.eql(
      "request.body.bid.volume_mwh should be integer"
    );
  });
  ...
  it("should failed with missing price", async () => {
    ...
    expect(res.status).to.eql(400);
    expect(res.body.message).to.eql(
      "request.body.bid should have required property 'price_eur_per_mwh'"
    );
  });
  ...
  it("should failed with non number price", async () => {
    const res = await chai
      ...
      .post(url)
      .send({
        bid: { type: "buy", price_eur_per_mwh: "toto", volume_mwh: 50 },
      });

    expect(res.status).to.eql(400);
    expect(res.body.message).to.eql(
      "request.body.bid.price_eur_per_mwh should be integer"
    );
  });
